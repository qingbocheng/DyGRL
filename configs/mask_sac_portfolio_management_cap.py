...
# rolling_split_path = '/root/quant-ml-qlib/Graph-EarnMore/datasets/ndx/features/AAPL.csv'
...
transition_shape = dict(
    state = dict(shape = (num_envs, num_stocks, days, num_features), type = "float32"),
    rep_state = dict(shape = (num_envs, num_stocks, embed_dim), type = "float32"),
    action = dict(shape = (num_envs, num_stocks+1), type = "float32"),
    node = dict(shape = (num_envs, num_stocks), type = "int32"),
    reward = dict(shape = (num_envs, ), type = "float32"),
    done = dict(shape = (num_envs, ), type = "float32"),
    next_rep_state = dict(shape = (num_envs, num_stocks, embed_dim), type = "float32"),
)

current_date = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
# current_date_1 = '2025-03-31-20-20-53'
current_date_2 = '2025-07-21-17-14-35'
...
#         mask_ratio_min = 0.05,
#         mask_ratio_max = 0.9,
#         mask_ratio_mu = 0.4,#0.6
#         mask_ratio_std = 0.2,#0.5
#     )

rep_net = dict(
        type = "GraphMaskTimeState_V2",
        in_dim=num_features,
        num_hidden=decoder_embed_dim,
        num_layers=2,
        num_dec_layers=2,
