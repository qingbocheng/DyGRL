...
import os
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:32"  # 更激进的内存分配策略
import sys
import gc
...
def main():
    ...
    #agent.rep.load_state_dict(agent.best_state_dict)
    #agent.rep.load_state_dict(torch.load("/root/quant-ml-qlib/EarnMore/configs/Sce_model_rep_weights.pth"))
    # tradeday_data = pd.read_csv(cfg.rolling_split_path)[['date']]
    # tradeday_data = tradeday_data[
    #     (tradeday_data['date'] >= '2022-08-06') & (tradeday_data['date'] <= '2025-03-01')].reset_index(drop=True)
    save = 0
    # agent.rep.load_state_dict(torch.load('/root/quant-ml-qlib/EarnMore/configs/checkpoints_sce/best/graphmae_best_14214.pth'))
    for episode in range(1, 1):
        # Rolling_data(val_envs, valid_sets, 0, episode, train_envs.envs[0])
        ...
        while True:
            ...
            #if save == 6:
            #    save = 0
            #    torch.save(agent.rep.state_dict(), f"/root/quant-ml-qlib/EarnMore/configs/checkpoints/graphmae_{episode}_{agent.global_step}.pth")
            if done[0]:
                break
        # agent.update_rep_net(lr_scheduler=agent.rep_scheduler, step=episode)
        # agent.rep.encoder.reweight()
        #agent.pre_explore_val(pre_train_envs,tradeday_data)
    # print(agent.best_state_dict)
    # agent.rep.load_state_dict(agent.best_state_dict)
    agent.rep.load_state_dict(torch.load('/root/quant-ml-qlib/Graph-EarnMore/configs/checkpoints/best/graphmae_best_6184.pth'))
    # torch.save(agent.rep.state_dict(), '/root/quant-ml-qlib/Graph-EarnMore/model_rep_weights.pth')
    agent.rep_train = False
    agent.rep.train_mae = False
    #train_envs.envs[0].mkt_observer.test(mode='pre-train')
...
def validate(environment, agent):
    ...
    # while True:
    #     done = agent.pre_explore_env(environment.envs[0], 64)
    #     if done:
    #         break
...
def rolling_train_split(data_path, start_trade, end_trade, train_size, valid_size, test_size, offset):
    ...
    while valid_end_index + 64 <= tradedays:
        train_sets.append([train_start_date, train_end_date])
        # valid_sets.append(['2022-08-06', '2025-10-01'])
        valid_sets.append(['2022-06-23', '2025-07-01'])
        # test_sets.append([test_start_date, test_end_date])
