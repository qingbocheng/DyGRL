...
import numpy as np
import gc
import mlfinlab.microstructural_features as mf
...
#        self.alpha_log = state_dict["alpha_log"]
    ...
    def explore_env(self, env, horizon_len: int) -> Tuple[Tensor, ...]:
        ...
        rep_states = build_storage((horizon_len, * self.transition_shape["rep_state"]["shape"]),
                               self.transition_shape["rep_state"]["type"], self.device)
        actions = build_storage((horizon_len, *self.transition_shape["action"]["shape"]),
                               self.transition_shape["action"]["type"], self.device)
        masks_node = build_storage((horizon_len, *self.transition_shape["node"]["shape"]),
                                self.transition_shape["node"]["type"], self.device)
        # masks_edge = build_storage((horizon_len, *self.transition_shape["edge"]["shape"]),
        #                         self.transition_shape["edge"]["type"], self.device)
        ...
        next_rep_states = build_storage((horizon_len, *self.transition_shape["next_rep_state"]["shape"]),
                               self.transition_shape["next_rep_state"]["type"], self.device)
        ...
        for t in range(horizon_len):
            ...
            # 训练时，掩码随机生成，不需要输入
            rep_state, node = self.rep.forward_state(env.envs[0].config.csp, env.envs[0].date_memory[-1],device=self.device)
            # action = self.get_action(rep_state)
            # 训练时，获取action，掩码需不需要传入？
            action,buffer_action = self.predict(self.get_action, rep_state, env.envs[0])
            # action,buffer_action = self.predict(self.get_action, rep_state, env.envs[0],mask)
            a_rlonly = np.array(action[0]) # [1, num_of_stocks] -> [num_of_stocks, ]
            a_rl = a_rlonly

            if np.sum(np.abs(a_rl)) == 0 or np.isnan(a_rl).any():
            ...
            if env.envs[0].config.enable_controller:
                a_final = RL_withController(a_rl=a_rl, a_buffer=np.array(buffer_action[0].detach().cpu().numpy()),env=env.envs[0], mask=node.cpu().numpy())
            else:
                a_final = a_rl
            # a_final = RL_withController(a_rl=a_rl, env=env.envs[0],mask=mask.cpu().numpy())
            a_final = a_final / np.sum(np.abs(a_final))
            a_final = np.array([a_final])
            ###################################################################
            states[t] = state
            rep_states[t] = rep_state
            next_state, reward, done, _ = env.step(a_final)
            next_rep_state, mask = self.rep.forward_state(env.envs[0].config.csp, env.envs[0].date_memory[-1],device=self.device,mask=node)
            ...
            dones[t] = done
            # next_states[t] = state
            next_rep_states[t] = next_rep_state
        ...
        return states, rep_states, actions, masks_node, rewards, dones, next_rep_states
    ...
    def pre_explore_env(self, env, horizon_len: int) -> Tuple[Tensor, ...]:
        ...
        for t in range(horizon_len):
            # b, e, n, d, f = state.shape
            # 训练时，掩码随机生成，不需要输入
            # ids_restore 打乱顺序后的位置对应的原始索引
            # rep_state, mask, ids_restore = self.rep.forward_state(
            #     rearrange(state, "b e n d f -> (b e) n d f", b=b, e=e))
            rep_state, mask = self.rep.forward_state(env.envs[0].config.csp, env.envs[0].date_memory[-1], device=self.device)
            # mask_3d = mask.unsqueeze(-1)
            # mask_applied = 1 - mask_3d   # 1 表示保留，0 表示清零
            # # 应用 mask
            # rep_state = rep_state * mask_applied
            ...
            # env.envs[0].env_mask = [mask.cpu().numpy()]
            a_rl = np.array([1/301]*301)
            next_state, reward, done, _ = env.step(np.array([a_rl]))
        self.update_rep_net(lr_scheduler=self.rep_scheduler, step=self.global_step)
        self.rep.encoder.reweight()
        # self.rep.encoder.reset_hidden_weights()
        self.global_step += 1
        torch.cuda.empty_cache()
        # self.last_state = next_state  # state.shape == (1, state_dim) for a single env.
        # print('explore_end\n',torch.cuda.memory_summary())
        return done
    
    def pre_explore_val(self, env, trade_date):

        mean_loss = 0
        self.rep.eval()
        with torch.no_grad():
            for env in range(4):
                for i in trade_date.values.tolist():
                    self.rep.forward_state_val(env.envs[0].config.csp, i[0], mask = env.envs[0].aux_stocks[env]["mask"], device=self.device)
                loss1 = torch.stack(self.rep.loss_list).mean()
                self.rep.loss_list = []
                mean_loss += loss1
                print("env val: ",env, loss1)
        if mean_loss < self.best_loss:
            self.best_loss = mean_loss
            self.best_state_dict = self.rep.state_dict()
        self.rep.train()

    def update_rep_net(self, states=None, masks=None, lr_scheduler=None, step=None):
        rep_loss = self.rep.forward_loss()
        if rep_loss < self.best_loss:
            self.best_loss = rep_loss
            self.best_state_dict = self.rep.state_dict()
            torch.save( self.rep.state_dict(), f"/root/quant-ml-qlib/Graph-EarnMore/configs/checkpoints/best/graphmae_best_{step}.pth")
    ...
    def update_net(self, buffer: ReplayBuffer,env=None) -> dict:
        ...
        for _ in tqdm(range(update_times),bar_format="update net batch " + "{bar:50}{percentage:3.0f}%|{elapsed}/{remaining}{postfix}"):
            ...
            obj_critic, state, nodes, rep_states = self.get_obj_critic(buffer, self.batch_size,env)
            ...
            # action_pg, log_prob = self.forward(self.get_action_logprob,rep_states,env,mask)
            action_pg, log_prob = self.forward(self.get_action_logprob,rep_states,env,nodes)
            ...
            if self.if_use_beta:
                beta_loss = self.get_object_beta(rep_states, nodes, env)
        ...
        if self.if_use_rep and self.rep_train:
            rep_loss = self.update_rep_net(state, nodes, lr_scheduler=self.rep_scheduler, step=self.global_step)
            rep_losses += rep_loss.item()
            self.rep.encoder.reweight()
            

        # print("act")
        # for name, p in self.act.named_parameters():
        #     print(f"{name:40s} grad  →", p.grad is not None,
        #           f"  |  norm={p.grad.norm():.4f}" if p.grad is not None else "")
        # print("cri")    
        # for name, p in self.cri.named_parameters():
        #     print(f"{name:40s} grad  →", p.grad is not None,
        #           f"  |  norm={p.grad.norm():.4f}" if p.grad is not None else "")
        # print("rep")      
        # for name, p in self.rep.named_parameters():
        #     print(f"{name:40s} grad  →", p.grad is not None,
        #           f"  |  norm={p.grad.norm():.4f}" if p.grad is not None else "")
    ...
    def validate_net(self, environment):
        ...
        for i in range(0,1):
            ...
            while True:
                # if i==1 and environment.envs[i].date_memory[-1] == '2025-01-22':
                #     ids = ids_1
                #     masks[i] = masks[4]
                ...
                rep_state, _ = self.rep.forward_state(environment.envs[i].config.csp, environment.envs[i].date_memory[-1],device=self.device,mask=masks[i].unsqueeze(0))
    ...
    def get_object_beta(self, rep_states: Tensor, mask: Tensor, env) -> Tensor:
        ...
        # weight,buffer_weight = self.predict(self.forward_action,rep_states,env,mask_bool)
        # weight = torch.tensor(weight, dtype=torch.float32,device=self.device,requires_grad=True)
        # buffer_weight = torch.tensor(buffer_weight, dtype=torch.float32,device=self.device,requires_grad=True)
        # tmp = weight * mask_bool
        beta_loss = (weight * mask_bool).sum(dim=1).mean()
        # beta_loss = weight.sum(dim=1).mean()
        # beta_loss = buffer_weight.sum(dim=1).mean()

        return beta_loss

    def get_obj_critic_raw(self, buffer, batch_size: int,env=None) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        with torch.no_grad():
            states, rep_states, actions, nodes, rewards, dones, rep_next_states = buffer.sample(batch_size)  # next_ss: next states

            if states.device != self.device:
                states = states.to(self.device)
                rep_states = rep_states.to(self.device)
                actions = actions.to(self.device)
                nodes = nodes.to(self.device)
                rewards = rewards.to(self.device)
                dones = dones.to(self.device)
                rep_next_states = rep_next_states.to(self.device)
            ...
            next_as, next_logprobs = self.forward(self.get_action_logprob, rep_next_states, env, nodes)
        ...
        return obj_critic, states, nodes, rep_states

    def get_obj_critic_per(self, buffer: ReplayBuffer, batch_size: int,env) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        with torch.no_grad():
            states, rep_states, actions, nodes, rewards, dones, rep_next_states, is_weights, is_indices = buffer.sample_for_per(batch_size)
            ...
            #                                          ids_restore=ids_restores)
            # next_as, next_logprobs = self.get_action_logprob(rep_next_states)
            # next_as, next_logprobs = self.predict(self.get_action_logprob,rep_next_states,env,mask)
            # next_as, next_logprobs = self.forward(self.get_action_logprob,rep_next_states,env,mask)
            next_as, next_logprobs = self.forward(self.get_action_logprob,rep_next_states, env, nodes)
            next_qs = self.cri_target.get_q_min(rep_next_states,next_as)
            alpha = self.alpha_log.exp().detach()
    ...
    def unscale_action(self, scaled_action: np.ndarray, env, masks=None) -> np.ndarray:
        ...
        if masks is not None:
            # masks = np.hstack(([[0]], masks))
            # zeros_column = np.zeros((scaled_action.shape[0], 1))
            # mask = np.concatenate([zeros_column, masks.detach().cpu().numpy()], axis=1)
            # mask = torch.cat([torch.zeros((scaled_action.shape[0], 1), device=self.device), masks], dim=1)
    ...
    def predict(self, get_action, state, env=None, mask=None):
        mask_sac_decision = get_action(state)
        ...
        daily_return_ay = np.array([np.array(row) for row in daily_return_ay_last])[:,-1]
        if env.config.enable_market_observer_CSP:
            ...
            mkt_decision = torch.as_tensor(daily_return_ay, dtype=torch.float32, device=self.device).unsqueeze(0)
            mkt_decision = torch.cat([torch.zeros((1, 1), device=self.device), mkt_decision], dim=1)
            lambda_r = 0.3
            #print(lambda_r)
            if mask is not None:
                mask = torch.cat([torch.zeros((state.shape[0], 1), device=self.device), mask], dim=1)
                # mkt_decision = mkt_decision * (1-mask)
                final_output = (mask_sac_decision.squeeze(-1) + lambda_r*mkt_decision).clamp(-1,1) # range [-1, 1]
                # final_output = (mask_sac_decision.squeeze(-1) * (1+mkt_decision)).clamp(-1,1)
                mask_sac_decision = final_output - mask * 10000
            else:
                mask_sac_decision = (mask_sac_decision.squeeze(-1) + lambda_r*mkt_decision).clamp(-1,1) # range [-1, 1]
                # mask_sac_decision = (mask_sac_decision.squeeze(-1) * (1+mkt_decision)).clamp(-1,1) # range [-1, 1]
        else:
            if mask is not None:
                if mask.shape[1] != mask_sac_decision.shape[1]:
                    mask = torch.cat([torch.zeros((state.shape[0], 1), device=self.device), mask], dim=1)
                mask_sac_decision = mask_sac_decision - mask*10000
            # final_output = mask_sac_decision.squeeze(-1).detach().cpu().numpy()
        #mask_sac_decision_buffer = torch.nn.Softmax(dim=1)(mask_sac_decision)
    ...
    def forward(self, get_action, state, env=None, mask=None):
        ...
        # if env.config.enable_market_observer_CSP:
        #     mkt_decision = torch.as_tensor(env.cur_hidden_vector_ay[-state.shape[0]:], dtype=torch.float32, device=self.device).squeeze(1)
        #     mkt_decision = torch.cat([torch.zeros((state.shape[0], 1), device=self.device), mkt_decision], dim=1)
        #     if mask is not None:
        #         mask = torch.cat([torch.zeros((state.shape[0], 1), device=self.device), mask], dim=1)
        #         # mkt_decision = mkt_decision * (1-mask)
        #         # final_output = (mask_sac_decision.squeeze(-1) + mkt_decision).clamp(-1,1)  # range [-1, 1]
        #         final_output = (mask_sac_decision.squeeze(-1) * (1+mkt_decision)).clamp(-1,1)
        #         mask_sac_decision = final_output - mask * 10000
        #     else:
        #         # mask_sac_decision = (mask_sac_decision.squeeze(-1) + mkt_decision).clamp(-1,1)  # range [-1, 1]
        #         mask_sac_decision = (mask_sac_decision.squeeze(-1) * (1 + mkt_decision)).clamp(-1, 1)  # range [-1, 1]
        # else:
        #     if mask is not None:
        #         if mask.shape[1] != mask_sac_decision.shape[1]:
        ...
        # unscaled_action = self.unscale_action_forward(actions,env)
        # # # 预测输出后处理
        # scaled_action = self.scale_action(unscaled_action,env)
        # buffer_action = scaled_action
        # # mask_sac_decision_prob -= (-buffer_action.pow(2) + 1.000001).log()
        weight = torch.nn.Softmax(dim=1)(mask_sac_decision)
    ...
    def compute_ids_restore(self, mask):
        # 获取保留 (mask == 0) 和掩盖 (mask == 1) 的索引
        ids_keep = torch.nonzero(mask == 0, as_tuple=True)[1]  # 保留部分的索引
        ids_masked = torch.nonzero(mask == 1, as_tuple=True)[1]  # 掩盖部分的索引
