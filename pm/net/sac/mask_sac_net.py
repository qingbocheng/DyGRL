...
import sys
sys.path.append('/root/quant-ml-qlib/EarnMore')
...
class ActorMaskSAC(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList(
            [
                Mlp(in_features=embed_dim, hidden_features=64, out_features=128, drop=0.01, act_layer=nn.GELU)
        ...
        self.decoder_pred = nn.Sequential(
            # nn.LayerNorm(128),
            Mlp(in_features=128, hidden_features=64, out_features=256, drop=0.01, act_layer=nn.GELU),
            Mlp(in_features=256, hidden_features=64, out_features=2, drop=0.01, act_layer=nn.GELU)
         )
    ...
    def get_action_logprob(self, x):
        ...
        weight = torch.tanh(logits)
        logprob = dist.log_prob(logits)
        logprob -= (-torch.tanh(weight).pow(2) + 1.000001).log()
...
class CriticMaskSAC(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList(
            [
                Mlp(in_features=embed_dim + 1, hidden_features=64, out_features=128, act_layer=nn.GELU, drop=0.01)
                for i in range(depth)
            ]
        )
