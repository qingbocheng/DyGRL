...
def get_action_wrapper(func,
    ...
    def get_action(x,
        ...
        if method == "softmax":
            pred = func(x, **kwargs)
            weight = pred / T
            # weight1 = torch.nn.Softmax(dim=1)(pred)
            # noise = (torch.randn_like(pred) * 0.1).clamp(-0.5, 0.5)
            # pred = pred + noise
            # if mask is not None:
            #     mask_bool = mask_bools(mask, x, pred)
            #     mask_bool = mask_bool * mask_value
            #     pred = pred - mask_bool
            # weight = torch.nn.Softmax(dim=1)(pred)
            # weight = F.softmax(pred, dim=-1)
            # weight = pred*(1-mask_bool)
            ...
            weight = torch.nn.Softmax(dim=1)(soft_pred)
        else:
