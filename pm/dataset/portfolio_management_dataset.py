...
class PortfolioManagementDataset():
    def __init__(self,
        ...
        self.features_name = pd.read_csv(self.root + '/datasets/ndx/masa/techIndicatorLst.csv')['0'].tolist()
    ...
    def _init_stocks_df(self):
        ...
        for stock in self.stocks:
            ...
            #df['ret_21'] = [ret_array[i - n:i].tolist() if i >= n else np.nan for i in range(len(ret_array))]
            # df = df[self.features_name + self.labels_name]
    ...
    def _load_market_data(self):
        df = pd.read_csv(self.root  + f'/datasets/{self.config.csp}/masa/Index.csv')
